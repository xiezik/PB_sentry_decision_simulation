...
#ifndef RM_COMMON_MATH_HPP
#define RM_COMMON_MATH_HPP
...
inline double Cross2D (const rm_common::Point2D &vector0,
                       const rm_common::Point2D &vector1) {
  return vector0.X() * vector1.Y() - vector0.Y() * vector1.X();
}

inline double Cross2D (const rm_common::Point2D &start_point,
                       const rm_common::Point2D &point0,
                       const rm_common::Point2D &point1) {
...
inline bool ConvexVerify(std::vector<rm_common::Point2D> &points) {
...
inline double PointDistance (const rm_common::Point2D &point0,
                             const rm_common::Point2D &point1) {
...
inline double PointToLineDistance (const rm_common::Point2D &point,
                                   const rm_common::LineSegment2D &line) {
...
inline bool CheckLineSegmentsIntersection2D(const rm_common::LineSegment2D &line0,
                                            const rm_common::LineSegment2D &line1,
                                            rm_common::Point2D *intersection = NULL) {
  double s_numer, t_numer, denom, t;
  ...
  rm_common::LineSegment2D aux(line1.Start(), line0.Start());
  ...
  t = t_numer / denom;
  if (intersection) {
    *intersection = line0.Start() + rm_common::Point2D(t * line0.DiffX(), t * line0.DiffY());
  }
...
inline double DistanceSegmentToSegment2D(const rm_common::LineSegment2D &line0, const rm_common::LineSegment2D &line1) {
...
inline double DistancePointToPolygon2D(const rm_common::Point2D &point, const rm_common::Polygon2D &polygon) {
  double dist = HUGE_VAL;
...
inline double DistanceSegmentToPolygon2D(const rm_common::LineSegment2D line,
                                         const rm_common::Polygon2D polygon) {
  double dist = HUGE_VAL;
...
inline double DistancePolygonToPolygon2D(const rm_common::Polygon2D &polygon0,
                                         const rm_common::Polygon2D &polygon1) {
  double dist = HUGE_VAL;
...
inline rm_common::Point2D PointRotateAroundPoint(const rm_common::Point2D source,
                                                   const rm_common::Point2D around,
                                                   double theta)
{
    rm_common::Point2D val((source.X() - around.X())*std::cos(theta) - (source.Y() - around.Y())*std::sin(theta) + around.X(),
...
inline int PointInRect(const rm_common::Point2D point,
                       const rm_common::Point2D rect_1,
                       const rm_common::Point2D rect_2)
{
...
{
  while(angle > rm_common::PI)
    angle -= 2*rm_common::PI;
  while(angle <= -rm_common::PI)
    angle += 2*rm_common::PI;
  return angle;
}
